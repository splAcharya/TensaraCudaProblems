...
static int run_profile() {
  ...
  const size_t H = 10000000;
  const int kernel_size = 7;
  const int stride = 2;
  const int padding = 3;
  const size_t H_out =
      avg_pool_output_size(H, kernel_size, stride, padding);
  const size_t input_bytes = H * sizeof(float);
  const size_t output_bytes = H_out * sizeof(float);
  const auto input = make_avg_pool_input(H);
  ...
  CUDA_CHECK(cudaMalloc(&d_input, input_bytes));
  CUDA_CHECK(cudaMalloc(&d_output, output_bytes));
  CUDA_CHECK(cudaMemcpy(d_input, input.data(), input_bytes,
  ...
  const double device_mib = static_cast<double>(
      input_bytes + output_bytes) / (1024.0 * 1024.0);
