...
// CPU reference for elementwise ReLU.
//
// input: row-major matrix flattened as a vector with shape (rows * cols)
// Returns output: row-major matrix flattened as a vector with the same shape
// rows/cols: carried by the caller's TestCase; this function only needs total
